# tf*idf
# tf - term freq.
# idf - inverted document freq.
# 1. Считаем tf
# 2. Считаем idf
# 3. Считаем tf-idf
# 4. Для каждого слова в каждом тексте
# 5. А потом берём 5 слов с минимальным значением
import os
import re
...
def preprocessing(text): # функция предобработки текста
    ...
    text_wo_punct = re.sub(tabs, ' ',text_wo_punct)# удаляем пунктуацию, приводим в нижний регистр
